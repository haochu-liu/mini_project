...
Set the number of leaf lineages to be 20, 100, 500. And set $\delta = 1, 10, 20$. Increase $\rho$ from 1 to 10. For every point, I simulated 10 times and take the mean value.

```{r}
#| echo: true
n_values <- c(20, 100, 500)
rho_values <- 1:10
...
for (i in 1:nrow(node_df)) {
  ...
  if (func_str == "sim_FSM_ARG(optimise=T)") {
    for (i in 1:10) {
      r <- sim_FSM_ARG(n, rho, 100, bacteria = TRUE, delta = delta,
                       node_max = 100000, optimise_recomb = TRUE)
      node_vec[i] <- length(r$node_height)
    }
  ...
  node_df$n_node[i] <- mean(node_vec)
