source("sim_gene/effective_R_n.r")
...
simbac_ARG <- function(n, rho, L, delta, node_max=1000, output_eff_R=FALSE) {
  ...
  node_probstart <- matrix(0, nrow=node_max, ncol=L) # node recomb starting prob
  ...
  node_probstart[1:n, ] <- matrix(rep(1/L, L), nrow=n, ncol=L, byrow=TRUE)
  ...
  while (k > 1) {
    ...
    if (p_coale == 1) {
      ...
      # update effective R
      list_eff_R <- effective_R(node_mat[node_index, site_include], delta,
                                sum(site_include), rho)
      node_eff_R[node_index] <- list_eff_R$R_eff
      node_probstart[node_index, site_include] <- list_eff_R$probstart
      ...
      site_include_index <- which(site_include)
      for (i in 1:sum(site_include)) {
        if (is.na(sum(node_mat[pool, site_include_index[i]]) == 1)) {
          print(site_include_index[i])
          print(site_include)
          return(node_mat[pool, ])
        }
        if (sum(node_mat[pool, site_include_index[i]]) == 1) {
          site_include[site_include_index[i]] <- FALSE
          node_probstart[pool, ] <- 0
          for (j in pool) {
            list_eff_R <- effective_R(node_mat[j, site_include], delta,
                                      sum(site_include), rho)
            node_eff_R[j] <- list_eff_R$R_eff
            node_probstart[j, site_include] <- list_eff_R$probstart
          }
          # print("exclude one site")
        }
      }
    } else {
      # recombination event
      leaf_node <- sample(pool, size=1, replace=FALSE, prob=node_eff_R[pool])
      leaf_probstart <- node_probstart[leaf_node, ]
      if (sum(leaf_probstart) )
      leaf_probstart <- cumsum(leaf_probstart / mean(leaf_probstart))

      repeat {
        x <- which(runif(1) < leaf_probstart)[1]
      ...
      node_eff_R[node_index] <- list_eff_R$R_eff
      node_probstart[node_index, site_include] <- list_eff_R$probstart
      list_eff_R <- effective_R(node_mat[node_index+1, site_include], delta,
                                sum(site_include), rho)
      node_eff_R[node_index+1] <- list_eff_R$R_eff
      node_probstart[node_index+1, site_include] <- list_eff_R$probstart
    ...
    if (max(edge_index, next_node, node_index) >= node_max - 1) {
      ...
      node_probstart <- rbind(node_probstart, matrix(0, nrow=node_max, ncol=L))
      node_max <- 2 * node_max
    }
  }
