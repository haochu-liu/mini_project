#' Input: ARG (simARG), mutation rate
...
sim_mutation <- function(ARG, theta) {
  ...
  for (i in nrow(ARG$edge):1) {
    ...
    parent_seq <- new_ARG$node$gene[[which(ARG$node$index==ARG$edge$node1[i])]]
    new_ARG$node$gene[[which(ARG$node$index==ARG$edge$node2[i])]] <- unique(sort(c(parent_seq,
      edge_mutation, new_ARG$node$gene[[which(ARG$node$index==ARG$edge$node2[i])]])))
  }
  # convert to string
